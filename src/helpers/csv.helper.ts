...
class CsvHelper {
	 ...
	 * @returns
	 */
	public static encode<T extends {}>(
		rows: T[],
		...
		;(async () => {
			...
			if (rows.length > 0) {
				...
				await this.writeRows(writer, rows, columns)
			}
			await writer.close()
		})()
	...
	static async writeRows<T>(
		...
		// write header row
		await write(
			utf8(columns.map((c) => this.encodeValue(c.label)).join(delimiter)),
		)
		...
		for (const row of rows) {
			...
			for (const { key, format } of columns) {
				s += (first ? '' : ',') +
					this.encodeValue(format(row[key], row))
				first = false
			}
			await write(utf8(s))
		}
